...
import azkaban.jobExecutor.JobExtend;
...
public class ProcessJob extends AbstractProcessJob {
  ...
  private String getEffectiveUser(final Props jobProps) {
    final JobExtend jobExt = new JobExtend();
    final Props azkProps = jobExt.loadConfigurationFromAzkabanHome();
    final String proxyEnable = azkProps.getString("executor.proxy.enable","false");
    String effectiveUser = null;
    String submitUser = "";

    if (!jobProps.containsKey(CommonJobProperties.SUBMIT_USER) && !jobProps.containsKey(JobProperties.USER_TO_PROXY)) {
        throw new RuntimeException(
        "Internal Error: No user.to.proxy or submit.user in the jobProps");
     }

    if (jobProps.containsKey(CommonJobProperties.SUBMIT_USER)) {
        submitUser = jobProps.getString(CommonJobProperties.SUBMIT_USER);
        effectiveUser = submitUser;
    }
    if (jobProps.containsKey(JobProperties.USER_TO_PROXY) && proxyEnable.trim().equals("true") ){
        String proxyUser = jobProps.getString(JobProperties.USER_TO_PROXY);
        boolean proxyValid = jobExt.validProxyUserFromDB(azkProps,submitUser,proxyUser);
        if (proxyValid) {
            effectiveUser = proxyUser;
        }else {
            warn(submitUser + ": can't execute job by proxy user "+proxyUser);
        }
     }
